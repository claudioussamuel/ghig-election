...
export default function Home() {
  ...
  const { user, loading, hasVoted, logOut, checkVoteStatus, getUserPin } = useAuth()
      ...
      <div className="container mx-auto px-4 py-4 sm:py-8">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-4 items-center w-full sm:w-auto">
            <div className="flex gap-2 sm:gap-4 flex-1 sm:flex-initial">
              <button
                ...
                className={`flex-1 sm:flex-initial px-4 sm:px-6 py-2 rounded-lg font-medium transition-all text-sm sm:text-base ${currentView === "vote"
        ...
        <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-card rounded-lg border border-border">
          <p className="text-xs sm:text-sm text-muted-foreground flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-0">
            <span>Logged in as: <span className="text-foreground font-medium">
              {getUserPin() ? `PIN: ${getUserPin()?.slice(0, 2)}****` : user?.email || 'Unknown'}
            </span></span>
